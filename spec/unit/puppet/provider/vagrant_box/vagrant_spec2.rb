...
describe Puppet::Type.type(:vagrant_box).provider(:vagrant) do
	...
	let :provider do
		described_class.new(resource).tap do |provider|
			# provider.class.stubs(:optional_commands).with(:composer).returns '/usr/bin/composer'
			provider.class.stubs(:command).with(:vagrant).returns '/usr/bin/vagrant'
		end
  end
  ...
  it 'abc' do
    ...
    local_variables
		instance_variables
		public_methods true # false zwroci pusta tablice
